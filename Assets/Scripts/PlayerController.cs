...
public class PlayerController : MonoBehaviour
{
   ...
   // private GameObject aimIndicatorInstance;
    ...
    public void Attack()
    {
        ...
        if (Input.GetMouseButtonUp(0) && currTime > createTime)
        {
            print("팽눌렀다1");
        ...
        else if (Input.GetMouseButtonUp(0) && currTime <= createTime)
        {
            aimIndicatorAttack.SetActive(false);
            //isAttacking = true;
            FastAttack();
    ...
    {
        StunEnemies();
        GameObject sk = Instantiate(skills);
        sk.transform.position = transform.position;
        sk.transform.up = transform.forward;
    ...
    void ReleaseAttack()
    {
        ...
        pf = gameObject.GetComponent<PlayerFire>();
